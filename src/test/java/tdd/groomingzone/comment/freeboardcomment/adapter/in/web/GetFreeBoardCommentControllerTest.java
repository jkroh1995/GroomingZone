...
import tdd.groomingzone.comment.freeboardcomment.controller.GetFreeBoardCommentController;
    ...
    void testGetFreeBoardComment() throws Exception {
        ...
        for (int i = 0; i < responseList.size(); i++) {
            mockMvc.perform(
                    ...
                    .andExpect(jsonPath("data[%d].content", i).value(responseList.get(i).content()))
                    .andExpect(jsonPath("data[%d].createdAt", i).value(responseList.get(i).createdAt().format(DateTimeFormatter.ofLocalizedDateTime(FormatStyle.MEDIUM))))
                    .andExpect(jsonPath("data[%d].modifiedAt", i).value(responseList.get(i).modifiedAt().format(DateTimeFormatter.ofLocalizedDateTime(FormatStyle.MEDIUM))))
